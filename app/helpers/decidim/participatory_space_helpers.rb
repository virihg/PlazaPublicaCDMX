...
module Decidim
  module ParticipatorySpaceHelpers
    ...
    def participatory_space_non_floating_help
      return if help_section.blank?

      content_tag "section", class: "row collapse" do
        concat(participatory_space_name)
        concat(non_floating_help(help_id) { translated_attribute(help_section).html_safe })
      end
    end

    def participatory_space_name
      content_tag "h3", class: "font-weight-bold" do
        I18n.t("models.decidim/#{help_id.to_s.singularize}.other", scope: "activerecord")
      end
    end

    def participatory_space_wrapper(&block)
      content_tag :main, class: "wrapper" do
        concat(participatory_space_non_floating_help)
